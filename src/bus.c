...
byte_t *get_sector(address_t address) {
    ...
    } else if (address >= BG_PALETTE_RAM_START &&
               address < BG_PALETTE_RAM_END) {
...
int write_half_word(address_t address, halfword_t *data) {
    ...
    *(halfword_t *)(sector_ptr + address - sector_offset) = *data;
...
int read_half_word(address_t address, halfword_t *data) {
    ...
    *data = *((halfword_t *)(sector_ptr + address - sector_offset));
    return 0;
}
