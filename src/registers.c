...
int init_registers(CpuRegister *cpuRegister) {
    ...
    uint32_t *p_reg_bank[] = {cpuRegister->register_data.fiq_registers, cpuRegister->register_data.svc_registers,
                              cpuRegister->register_data.abt_registers, cpuRegister->register_data.irq_registers,
...
int write_user_register(uint8_t reg_number, uint32_t value, CpuRegister *cpu_reg) {
    if (reg_number >= 16) return -1;
    cpu_reg->register_data.registers[reg_number] = value;
    return 0;
}
